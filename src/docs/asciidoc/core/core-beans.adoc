[[beans]]
= The IoC_BeanFactory Container

This chapter covers Spring's Inversion of Control (IoC_BeanFactory) container.




[[beans-introduction]]
== Introduction to the Spring IoC_BeanFactory Container and Beans

This chapter covers the Spring Framework implementation of the Inversion of Control
(IoC_BeanFactory) principle. IoC_BeanFactory is also known as dependency injection (DI). It is a process whereby
...
for Spring Framework's IoC_BeanFactory container. The
...
in this chapter in descriptions of Spring's IoC_BeanFactory container. For more information on using
...
by the Spring IoC_BeanFactory container are called beans. A bean is an object that is
instantiated, assembled, and managed by a Spring IoC_BeanFactory container. Otherwise, a
...
The `org.springframework.context.ApplicationContext` interface represents the Spring IoC_BeanFactory
...
more instances of a Spring IoC_BeanFactory container. For example, in a web application scenario, a
...
.The Spring IoC_BeanFactory container
...
As the preceding diagram shows, the Spring IoC_BeanFactory container consumes a form of
...
Spring IoC_BeanFactory container.

NOTE: XML-based metadata is not the only allowed form of configuration metadata.
The Spring IoC_BeanFactory container itself is totally decoupled from the format in which this
...
the control of an IoC_BeanFactory container. See <<aop-atconfigurable,Using AspectJ to
...
[NOTE]
====
After you learn about Spring's IoC_BeanFactory container, you may want to know more about Spring's
...
A Spring IoC_BeanFactory container manages one or more beans. These beans are created with the
...
class should suffice. However, depending on what type of IoC_BeanFactory you use for that specific
bean, you may need a default (empty) constructor.

The Spring IoC_BeanFactory container can manage virtually any class you want it to manage. It is
...
load an entire Spring IoC_BeanFactory container instance.
...
beans for classes A and B to be injected into each other, the Spring IoC_BeanFactory container
...
bean B, the Spring IoC_BeanFactory container completely configures bean B prior to invoking the
...
lazy-initialized. A lazy-initialized bean tells the IoC_BeanFactory container to create a bean
...
Spring Framework. Method Injection, a somewhat advanced feature of the Spring IoC_BeanFactory
...
| (Default) Scopes a single bean definition to a single object instance for each Spring IoC_BeanFactory
  container.
...
singleton, the Spring IoC_BeanFactory container creates exactly one instance of the object
...
`XmlWebApplicationContext`). If you use these scopes with regular Spring IoC_BeanFactory containers,
...
The Spring IoC_BeanFactory container manages not only the instantiation of your objects (beans),
...
definition. The Spring IoC_BeanFactory container calls that method when the bean is created (and in
...
attribute causes the Spring IoC_BeanFactory container to recognize a method called `init` on the bean
...
==== Shutting Down the Spring IoC_BeanFactory Container Gracefully in Non-Web Applications
...
the Spring IoC_BeanFactory container when the relevant web application is shut down.
====

If you use Spring's IoC_BeanFactory container in a non-web application environment (for
...
implementation classes. Instead, the Spring IoC_BeanFactory container can be extended by plugging in
...
the Spring IoC_BeanFactory container instantiates a bean instance and then `BeanPostProcessor`
...
implementation is a common means of extending the Spring IoC_BeanFactory container. An example is
...
That is, the Spring IoC_BeanFactory container lets a `BeanFactoryPostProcessor` read the
...
The `FactoryBean` interface is a point of pluggability into the Spring IoC_BeanFactory container's
...
Spring IoC_BeanFactory container. For example, Spring 2.5 introduced an annotation-based approach to
...
initializes a new object to be managed by the Spring IoC_BeanFactory container. For those familiar
...
The `BeanFactory` API provides the underlying basis for Spring's IoC_BeanFactory functionality.
